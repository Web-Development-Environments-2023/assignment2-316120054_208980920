...
function resetElements() {
    ...
    canvasWidth = w; // store the width of the canvas
...
}//end updateLeaderboard function
...
function update_ship_shot(){
    //this function update the ship shot y position
    var to_brake=false;
    if(cannonballOnScreen){
...
}// end update_ship_shot function
function update_direction_enemy(){
    // this function update the enemy direction
    for(var i=0;i<ROWS;i++){
        for(var j=0;j<COLUMN;j++){
...
function end_game(){
    // check if the game is over for several reasons
    if(liveRemaining==0){
